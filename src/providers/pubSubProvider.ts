...
const pubSubProvider: IPubSubProvider = {
    publish: async (eventType: string, event: Message | any, bot: IDiscordBot) => {
        logger(`Bot: ${bot.name} is publishing event of type ${eventType}`, null, LogLevel.INFO);
        ...
        if (!isTruthy(eventType)) {
            logger(`Event type is falsy.`, null, LogLevel.ERROR);
            return;
        }
    ...
    subscribe: async (subscriber: ISubscriber) => {
        logger(`New AI subscriber: ${subscriber.name} to Discord Bot: ${subscriber.bot.name} for eventType: ${subscriber.eventType}`, null, LogLevel.INFO);
    ...
    unsubscribe: async (name: string, eventType: string) => {
        logger(`Unsubscribing subscriber: ${name} from eventType: ${eventType}`, null, LogLevel.INFO);
    ...
    unsubscribeAll: async (name: string) => {
        logger(`Unsubscribing subscriber: ${name} from all events.`, null, LogLevel.INFO);
