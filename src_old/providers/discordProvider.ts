...
export const startDiscordClient = async (bot: IDiscordBot) => {
    ...
    if (!isTruthy(discordClient)) {
        await setupClient(bot)
            ...
            .catch((err: any) => {
                logger(`Error starting Discord client:`, err, LogLevel.ERROR);
    ...
    if (!isTruthy(newClient)) {
        logger(`Could not get new Discord client.`, null, LogLevel.ERROR);
...
const setupClient = async (bot: IDiscordBot) => {
    try {
        ...
        logger(`Discord bot ${bot.name} logged in!`, null, LogLevel.INFO);
    } catch (err: any) {
...
const startEventListener = async (bot: IDiscordBot) => {
    ...
    myClient.on(Events.MessageCreate, async (message: Message) => { // TODO: Look at a generic listener for all events.
        if (!isTruthy(bot.tag)) {
            logger(`Bot tag was falsy! Please fill it in: ${message.content}`, null, LogLevel.ERROR);
            return;
        }
        ...
        logger(`Discord Bot: ${bot.name} received a message: ${message.content.replace(`${bot.tag} `, "")}`, null, LogLevel.INFO);
        
        if (!isTruthy(message.content)) {
            logger(`Message content was falsy.`, null, LogLevel.ERROR);
    ...
    logger(`Discord bot ${bot.name} event listener started!`, null, LogLevel.INFO);
}